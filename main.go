...
import (
	...
	"github.com/lalrochhara/Sangtei/Sangtei/config"
	...
	"github.com/lalrochhara/Sangtei/Sangtei"
)
...
func main() {
	...
	// Loading Modules
	Sangtei.LoadModules(dispatcher)

	// list modules from modules dir
	log.Infof(
		fmt.Sprintf(
			"[Modules] Loaded modules: %s", Sangtei.ListModules(),
		),
	)

	// send message to log group
	_, err = b.SendMessage(config.MessageDump,
		fmt.Sprintf("<b>Started Bot!</b>\n<b>Mode:</b> %s\n<b>Loaded Modules:</b>\n%s", config.WorkingMode, Sangtei.ListModules()),
